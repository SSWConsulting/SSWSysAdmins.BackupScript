    ...
    if ($warnings.Count -eq 0) {
        ...
        if ($compileResults.Errors.Count -gt 0) {
            ...
            if (! $Anonymous) {
                ...
                #$pass = cat C:\temp\pass.txt | convertto-securestring                                                            
                #$mycred = new-object -typename System.Management.Automation.PSCredential -argumentlist "test",$pass                
                #$proxy.Credentials = $mycred


                #$Username = "ssw2000\danielhyles" 
                #$Password = ConvertTo-SecureString 'PASSWORD' -AsPlainText -Force 
                #$proxy.Credentials = New-Object System.Management.Automation.PSCredential $Username, $Password 
